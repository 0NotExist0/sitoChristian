/**
 * CATALOG MANAGER - Versione 3.4
 * Sistema "Divine Per-Card Loading" per Luxury Doors
 */
const CatalogManager = {
    init() {
        this.grid = document.querySelector(".collections__grid");
        // Riferimento al loader di sezione (opzionale se vogliamo tenerlo come sipario iniziale)
        ...
        this.grid.innerHTML = "";
        this.grid.style.opacity = "1"; // La griglia è pronta a ricevere le card
    ...
    buildCards() {
        ...
        categories.forEach(([categoryKey, dataNode]) => {
            ...
            // Iniettiamo il Loader Divino direttamente nella card
            card.innerHTML = `
            ...
            // Gestione caricamento immagini con evento onload
            ...
            previewImages.forEach((src, idx) => {
                ...
                img.onload = () => {
                    ...
                    // Se la prima immagine (quella visibile) è carica, rimuoviamo il loader
                    if (idx === 0 && !primaryImageLoaded) {
                        primaryImageLoaded = true;
                        setTimeout(() => {
                            loader.style.opacity = "0";
                            setTimeout(() => loader.style.display = "none", 500);
                        }, 400); // Delay estetico per mostrare l'animazione del fulmine
                    }
                };
    ...
    startSlideshows() {
        document.querySelectorAll('.collection-card').forEach(card => {
            ...
            let current = 0;
            setInterval(() => {
                // Cambia slide solo se l'immagine successiva è stata caricata
